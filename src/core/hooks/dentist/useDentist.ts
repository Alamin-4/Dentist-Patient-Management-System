...
export function useInvalidateVerification() {
  const queryClient = useQueryClient();
  return () => {
    queryClient.invalidateQueries({ queryKey: ["dentistVerificationProgress"] });
...
export function useProfessionalDetailsMutation() {
  const queryClient = useQueryClient();
...
export function useStepOneMutation() {
  const invalidate = useInvalidateVerification();
  return useMutation({
...
export function useStepTwoMutation() {
  const invalidate = useInvalidateVerification();
  return useMutation({
...
export function useStepThreeMutation() {
  const invalidate = useInvalidateVerification();
  return useMutation({
    ...
    mutationFn: async (data: StepThreeI) => {
      ...
      let lat = Number(data.clinic_address?.lat);
      let lng = Number(data.clinic_address?.lng);
...
export function useGlobalProcedureList() {
  return useQuery({
...
export function useDentistProceduresList() {
  return useQuery({
