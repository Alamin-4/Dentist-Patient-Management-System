...
export default function ReviewPlanPage() {
  ...
  const specialty = plan.dentist?.specialty || "Dentist";
  const avatarSrc = plan.dentist?.profileImageUrl || "/images/dentist.png";
  ...
  const handleAccept = () => {
    ...
    decisionMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Treatment plan accepted successfully!");
          setShowSuccess(true);
        },
        onError: (err: any) => {
          toast.error(err?.response?.data?.message || "Failed to accept treatment plan.");
        },
      }
    );
  };
  ...
  const scheduledDate = plan.consultation?.scheduledDate
    ? new Date(plan.consultation.scheduledDate).toLocaleDateString()
    : "Not Scheduled";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          <SectionCard>
            ...
            <div className="flex items-center justify-between p-4 border border-slate-100 rounded-lg mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#475569]">{specialty}</p>
                </div>
              </div>
              ...
              <div className="divide-y divide-slate-50">
                {plan.lineItems?.map((item: any, idx: number) => (
                  <div key={idx} className="flex justify-between px-4 py-4 text-sm">
                    <span className="text-[#64748B]">{item.globalProcedure?.name || "Procedure"}</span>
            ...
            <div className="mt-6 bg-[#F0F9FF] p-5 rounded-lg border border-[#B3D8FF]">
              ...
              <p className="text-[#203A55] text-sm leading-relaxed">
                ...
                {totalEstimate.toLocaleString()}. If {doctorName}&apos;s
          ...
          <div className="flex items-start gap-3 px-2 py-4">
            ...
            />
            <label htmlFor="consent" className="text-slate-500 text-sm leading-relaxed cursor-pointer font-medium">
              I have reviewed the treatment plan and understand the estimate range of $
              {totalEstimate.toLocaleString()}, with the No Surprise Guarantee
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[#6B7280] text-sm font-semibold">
                  Appointment Date :{" "}
                  <span className="text-black font-bold">{scheduledDate}</span>
                </p>
              </div>
              <div className="flex items-center gap-3">
                <div className="size-4 rounded-full bg-[#0E3E65] shrink-0" />
                <p className="text-[#6B7280] text-sm font-semibold">
                  Treatment takes :{" "}
                  <span className="text-black font-bold">As planned once you arrive</span>
                </p>
              </div>
            </div>
            ...
            <div className="mt-8 bg-slate-50 p-6 rounded-lg border border-slate-100">
              ...
              {signatureData ? (
                ...
                <button
                  type="button"
                  onClick={() => setSignModalOpen(true)}
                  className="w-full h-32 bg-white border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center hover:border-[#0F3659] transition-all cursor-pointer"
                >
      ...
      <div className="mt-8 border-t border-slate-200 pt-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 bg-[#F0F9FF] px-4 py-3 rounded-lg border border-[#B3D8FF]">
            ...
            <p className="text-[#0369A1] text-xs font-semibold">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              ...
              className="flex-1 md:flex-none px-8 py-3.5 rounded-lg border border-slate-300 font-bold text-[#EF4444] hover:bg-red-50 transition-all cursor-pointer"
            >
              Reject
            </button>
            <button
              ...
              disabled={!agreed || !signatureData || decisionMutation.isPending}
              className={cn(
                "flex-1 md:flex-none px-10 py-3.5 rounded-lg font-bold text-[15px] transition-all cursor-pointer",
                agreed && signatureData && !decisionMutation.isPending
                  ? "bg-[#0F3659] text-white hover:bg-[#0A2640] active:scale-95"
                  : "bg-slate-200 text-slate-400 cursor-not-allowed"
              )}
            >
              {decisionMutation.isPending ? "Processing..." : `Pay $${totalEstimate.toLocaleString()} securely`}
            </button>
          </div>
        </div>
      </div>
