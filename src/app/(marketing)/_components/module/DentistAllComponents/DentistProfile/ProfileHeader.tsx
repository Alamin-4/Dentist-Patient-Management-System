...
export default function ProfileHeader() {
  ...
  return (
    <div className="flex items-center gap-8 rounded-lg border border-slate-100 bg-white p-1 px-4 shadow-sm">
      {tabs.map((tab) => (
        <button
          key={tab.name}
          className={`flex items-center gap-2 border-b-4 py-4 text-sm font-bold transition-all ${tab.active
