...
function PatientCard({ patient, mode, onViewDetails }: PatientCardProps) {
  ...
  return (
    <article className="group w-full rounded-lg border border-border bg-card p-4 shadow-sm transition-all hover:border-border/80 hover:shadow-md sm:p-5">
      <div className="flex flex-col gap-5 lg:grid lg:grid-cols-[minmax(0,2.2fr)_minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,0.9fr)_auto] lg:items-center lg:gap-6">
        ...
        <div>
          ...
          <span
            className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${mode === "bookings"
        ...
        <div className="lg:justify-self-end">
          <button
            ...
            className="inline-flex h-10 w-full items-center justify-center rounded-lg border border-[#0F4C81] bg-card px-5 text-sm font-medium text-[#0F4C81] transition-all hover:bg-[#0F4C81] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F4C81]/30 sm:w-auto"
          >
            View Details
          </button>
        </div>
