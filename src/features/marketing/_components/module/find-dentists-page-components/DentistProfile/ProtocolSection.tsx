export default function ProtocolSection({
  ...
  if (isLoading) {
    return (
      <section id="protocols" className="rounded-lg border border-slate-200 bg-white p-6">
        <div className="h-8 w-64 bg-slate-200 rounded-md animate-pulse mb-6" />
  ...
  const protocols = [
    {
      ...
      value: dentistLicense?.registrationNumber || "",
    },
    {
      label: "Registration Authority",
      value: dentistLicense?.registrationAuthority || "",
    },
    {
      label: "License Document",
      value: dentistLicense?.licenseDocument
        ? (dentistLicense.isVerified ? "Verified" : "Pending Verification")
        : "",
    },
    {
      label: "JCI Certificate",
      value: dentistOperations?.jciCertificate ? "Verified" : "",
    },
    {
      label: "Walkthrough Video",
      value: dentistOperations?.walkthroughVideo ? "Submitted" : "",
    },
    {
      label: "Guarantee Agreement",
      value: dentistOperations?.agreedToGuarantee ? "Agreed" : "",
    },
  ];
  ...
  return (
    <section id="protocols" className="rounded-lg border border-slate-200 bg-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-12">
        {protocols.map((p, i) => {
          const hasValue = Boolean(p.value);
